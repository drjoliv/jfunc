...
class F5Composed<A,B,C,D,E,F,G> implements F5<A,B,C,D,E,G>{
  ...
  private final F1<? super F, G> fn;

  private F5Composed(F5<A, B, C, D, E, F> f5, F1<? super F, G> fn) {
    this.f5 = f5;
    this.fn = fn;
  }

  @Override
  public <H> F5<A, B, C, D, E, H> map(F1<? super G, H> fn) {
    return doMap(f5,this.fn.then(fn));
  }

   static <A, B, C, D, E, F, G> F5<A, B, C, D, E, G> doMap(F5<A, B, C, D, E, F> f5, F1<? super F, G> fn) {
